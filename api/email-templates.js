...
/**
 * Vercel Serverless Handler para servir previsualizaciones en vivo
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    const sampleStore = {
      storeName: req.query.store_name || 'Pizzería Don Antonio',
      storeId: req.query.store_id || 'pizzeria-don-antonio',
      resetLink: 'https://daletepido.com.ar/alta-usuario.html?action=login&reset_store=pizzeria-don-antonio'
    };

    const templates = {
      1: () => getWelcomeEmail(sampleStore),
      2: () => getResetPasswordEmail(sampleStore),
      3: () => getTrialReminder7DaysEmail(sampleStore),
      4: () => getTrialReminder3DaysEmail(sampleStore),
      5: () => getTrialEndedEmail(sampleStore)
    };

    const getTemplate = templates[templateId] || templates[1];
    const emailData = getTemplate();
    ...
    return res.status(200).send(emailData.html);
  } catch (err) {
    console.error('Error rendering email template preview:', err);
