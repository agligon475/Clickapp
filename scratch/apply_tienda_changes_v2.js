...
// 1. Add Coupon UI in Drawer Footer
...
if (html.includes(summaryTarget)) {
  ...
  console.log('Summary target not found or already replaced');
}
...
if (html.includes(cartTotalTarget)) {
  ...
  console.log('cartTotal target not found or already replaced');
}
...
if (html.includes(updateCartUITarget)) {
  ...
  console.log('✔ updateCartUI updated');
} else {
  console.log('updateCartUI target not found or already replaced');
}

// 4. Update enviarWhatsApp to include coupon info and increment used count
const msgTarget = `\`💰 *Total:* \${totalFormatted}\`,`;
const msgReplacement = `window.APPLIED_COUPON ? \`🎁 *Cupón:* \${window.APPLIED_COUPON.code} (-\${window.APPLIED_COUPON.discount_percentage}%) → Ahorro: -$\${getCartDiscountAmount().toLocaleString('es-AR')}\` : null,
    \`💰 *Total:* \${totalFormatted}\`,`;

if (html.includes(msgTarget) && !html.includes("🎁 *Cupón:*")) {
  html = html.replace(msgTarget, msgReplacement);
  console.log('✔ WhatsApp msg updated with coupon line');

  // Increment usage count logic in enviarWhatsApp
  const whatsappEndTarget = `const url = 'https://api.whatsapp.com/send?phone=' + cleanPhone + '&text=' + encodeURIComponent(text);`;
  const whatsappEndReplacement = `if (window.APPLIED_COUPON && window.STORE_COUPONS) {
    ...
    if (cObj) {
      ...
      fetchSupabase(\`/rest/v1/company_settings?store_id=eq.\${encodeURIComponent(STORE_ID)}\`, {
        ...
        body: JSON.stringify({ billing_info: { ...(window.STORE_COUPONS_RAW_BINFO || {}), coupons: window.STORE_COUPONS } })
      }).catch(() => {});
    }
  }\n  ${whatsappEndTarget}`;

  if (html.includes(whatsappEndTarget)) {
    html = html.replace(whatsappEndTarget, whatsappEndReplacement);
    console.log('✔ Usage count increment added to enviarWhatsApp');
  }
}

fs.writeFileSync('tienda.html', html, 'utf8');
console.log('Tienda changes v2 applied successfully.');
