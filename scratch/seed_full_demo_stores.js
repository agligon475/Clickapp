...
const storeNoutacc = {
  ...
  products: [
    ...
    {
      nombre: 'Porción de Tortón Red Velvet (Sin TACC)',
      precio: 4600,
      stock: 10,
...
async function seedStore(storeData) {
  ...
  // 2. Clear old products & categories
  ...
  for (const catObj of storeData.categories) {
    ...
    if (!catRes.ok) {
      console.error(`Error inserting category ${catObj.name}:`, await catRes.text());
    }
  ...
  for (const prod of storeData.products) {
    ...
    if (res.ok) {
      ...
      console.error(`Error inserting ${prod.nombre}:`, await res.text());
    }
...
async function main() {
  ...
  console.log('\n¡TODAS LAS TIENDAS DEMO FUERON POBLADAS CON ÉXITO!');
}
